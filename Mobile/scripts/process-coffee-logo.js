...
const inputPath = path.join(__dirname, '../assets/images/ChatGPT Image Aug 15, 2026, 01_12_13 PM.png');
...
fs.createReadStream(inputPath)
  ...
  .on('parsed', function () {
    ...
    // 1. Convert outer black pixels to transparent
    for (let y = 0; y < src.height; y++) {
      for (let x = 0; x < src.width; x++) {
        ...
        // If pixel is black background (outside book)
        if (r < 25 && g < 25 && b < 25) {
          src.data[idx + 3] = 0; // Make transparent
        }
      }
    }

    // 2. Find bounding box of non-transparent artwork
    ...
    console.log(`Coffee BookWorm artwork dimensions: ${artWidth}x${artHeight}`);
    ...
    console.log('Successfully generated transparent coffee splash logo!');
  });
